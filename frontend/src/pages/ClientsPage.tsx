...
export function ClientsPage() {
  ...
  const openEdit = (c: Client) => {
    setEditing(c); setName(c.name); setEmail(c.email); setPhone(c.phone)
    setShowForm(true)
  }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (editing) {
      updateMutation.mutate({ id: editing.id, name, email, phone })
    } else {
    ...
    <Layout>
      <div className="flex justify-between items-center mb-6">
        ...
        <Button onClick={() => setShowForm(true)}>
          <Plus size={16} className="inline mr-1" /> New Client
        </Button>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {isLoading ? (
          ...
          <p className="p-6 text-gray-500">No clients yet. Add one above.</p>
        ) : (
